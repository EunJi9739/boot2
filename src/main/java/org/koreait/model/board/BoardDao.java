...
public class BoardDao {
    ...
    public void insert(BoardForm boardForm){
        ...
        jdbcTemplate.update(conn -> {
            ...
            ps.setString(1, boardForm.getSubject());
            ps.setString(2, boardForm.getContent());
    ...
    public void processStat(){
        try {
            String sql = "SELECT COUNT(*), SUBSTR(REGDT, 1, 13) FROM BOARDDATA WHERE REGDT >= REGDT -1 GROUP BY SUBSTR(REGDT, 1, 13)";
            List<Board> stats = jdbcTemplate.query(sql, this::mapper);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
